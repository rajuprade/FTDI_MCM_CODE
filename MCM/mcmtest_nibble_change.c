...
Program is modified as problem occurs because of BIG endian Little endian format. Dt 8.2.1012
...
{
   int i;
    unsigned char Argument[64],temp;
...
char buff[64]={0,0xd,0,0,1,0,5,0,3,0,0,0,0,0}; 
 printf("\nEnter Dig Mask 32 (xx xx xx xx):\t");
      for (i=0;i<4;i++) 
  {
      scanf("%x", &Argument[i]);
       temp = Argument[i];
       Argument[i] = Argument[i]<<4;
   temp = temp>>4;
   Argument[i] = Argument[i] | temp;
   printf ("TEMP for nibble changing 0x%2X\n",Argument[i]);
...
void set_dmask_16b(char *buf_ptr0)
{
   int i;
    unsigned char Argument[64],temp;
   char buff[]={0,0xb,0,0,1,0,3,0,2,0,0,0};
