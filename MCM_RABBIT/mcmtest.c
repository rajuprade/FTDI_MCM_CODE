...
#include<time.h>
#include<fcntl.h>
#include "/home/acer1/Hello/ftd2xx.h"
      ...
      //                      printf(" DWRXSIZE is %d\n",dwRxSize);
	   	  ...
	   	  if(dwRxSize == 82)
                  {  
                     ...
                     //fprintf(stderr,"==========> FE Monitoring Data =================>\n"); 
                   // fprintf(f,"==========> TIME %s ========================>\n",ctime(&tim)); 
                  ...
                  {  
                    // fprintf(stderr,"==========> FE Monitoring Data 46 =================>\n"); 
                    // fprintf(f,"==========> TIME %s ========================>\n",ctime(&tim));
                 ...
                 fprintf(f,"\n"); 
                  ...
                  {  
                     //fprintf(stderr,"==========> COMMON BOX Monitoring Data =================>\n"); 
                   ...
                   // fprintf(stderr,"==========> COMMON Monitoring Done ===========>\n\n\n"); 
                 }
                   else
                   {
                     for(j = 0; j < dwRxSize; j++) 
...
void Convert(int offset, char mask[8])
{
  int j;
...
void Mask64toArg(char *buf_ptr0,char mask0[8], char mask1[8], char mask2[8], char mask3[8])
{
...
void Mask32toArg(char *buf_ptr0,char mask0[8], char mask1[8])
{
  Convert(0, mask0); Convert(2, mask1);
...
void SET_FE_Rabbit(char *buf_ptr0)
{
    int set_para[23];
    char mask[4][8];    
...
/*     control word for setting NOISE CAL LEVEL in front end box.*/
  ...
  /*   sprintf(mask[0], "07%02x",data1);
    ...
    sprintf(mask[0], "70%02x",FEDAT_OFF);
     sprintf(mask[1], "f0%02x",FEDAT_OFF);
     *buf_ptr0= 2;
     Mask32toArg(buf_ptr0,mask[0],mask[1]);  // 22-08-2013:This will keep MCM-5 on
...
void repeat_fe_box_mon(char *buf_ptr0,unsigned char b[2])
{
     int i;
     unsigned char Argument[64];
    char buff[32]={0,0xa,0,0,7,0,2,0,0,0,0xED };
...
void REPEAT_FE_BOX_MONITORING_CMD(char *buf_ptr0)
{
   char path[200],str[100];
  int itr,i,s;
  unsigned char box[2];
  fprintf(stderr," Enter the name of file\n");
  scanf("%s",str);
  sprintf(path,"/home/acer1/mem_raw/%s",str);
  f= fopen(path,"w");
   if(f == NULL)
   fprintf(stderr,"CANNOT OPEN FILE\n");
  printf(" How many time you want to send FE BOX MONITORING Command\n");
  scanf("%d",&itr);
  fprintf(stderr,"Enter the FE box number\n");
   scanf("%d",(int *)&box);
  fprintf(stderr,"Enter the time delay in seconds\n");
   scanf("%d",&s);
  ...
  { 
     //sleep(s);
     fprintf(f,"\n Serial number : %d\n",i);
     repeat_fe_box_mon(buf_ptr0,box);
      fflush(f);
     sleep(s);
   }
  fclose(f);
...
void REPEAT_COMMON_BOX_MONITORING_CMD(char *buf_ptr0)
{
   char path[200],str[100];
  int itr,i,s;
   fprintf(stderr," Enter the name of file\n");
  ...
  fp= fopen(path,"w");
   if(fp == NULL)
   fprintf(stderr,"CANNOT OPEN FILE\n");
  printf(" How many time you want to send COMMON BOX MONITORING Command\n");
  scanf("%d",&itr);
  fprintf(stderr,"Enter the time delay seconds\n");
   scanf("%d",&s);
   for(i=0;i<55;i++)
   {
     fprintf(fp,"Ch[%d]\t",i); fflush(fp);
   }
  for(i=0;i<itr;i++)
  { 
     fprintf(fp,"\n %d\t",i);
     common_box_mon(buf_ptr0);
     fflush(fp);
     sleep(s);
 }
fclose(fp);
...
{                                     
 switch(cmd)
 {
  ...
  case 23 : SET_FE_Rabbit(buf_ptr0);break;
  case 24 : REPEAT_NULL_CMD(buf_ptr0);break;
  case 25 : REPEAT_FE_BOX_MONITORING_CMD(buf_ptr0);break;
  case 26 : REPEAT_COMMON_BOX_MONITORING_CMD(buf_ptr0);break;
  default : break;
 }
}
...
int main(int argc, char *argv[])
{
 ...
 //for (i=0;i<3;i++) {
//for(;;){
/* f = fopen("/home/frontend/guest/Desktop/FE.txt","a+");
 fp = fopen("/home/frontend/guest/Desktop/CB.txt","a+");
 if(f == NULL)
 fprintf(stderr,"CANNOT OPEN FILE\n");*/
  int ret=-1;
 ...
 printf("2 : set_idle_mode\n");
  ...
  printf("10: read_version\n");
   ...
   printf("20: set_mean_mode\n");
    printf("21: SET_LO\n");
    printf("22: SET_FE\n");
    printf("23: SET_FE_RABBIT\n");
    printf("24: REPEAT_NULL_CMD\n");
    printf("25: REPEAT_FE_BOX_MONITORING_MD\n");
    printf("26: REPEAT_COMMON_BOX_MONITORING_CMD\n");
    printf("27: EXIT\n");
